...
@class WZSegmentItem;
@protocol WZSegmentItemDelegate
- (void)ItemStateChanged:(WZSegmentItem *)item index:(NSInteger)index isSelected:(BOOL)isSelected;
@end

@interface WZSegmentItem : UIView
...
@end
#pragma mark - WZSegmentView
@interface WZSegmentView()
    ...
    if (self) {
        ...
        if (items.count >= 2) {
            for (NSInteger i =0; i<items.count; i++) {
                WZSegmentItem *item = [[WZSegmentItem alloc] initWithFrame:CGRectMake(init_x, init_y, itemWidth, itemHeight)
...
- (void)setTintColor:(UIColor *)tintColor
{
    ...
    if (_tintColor != tintColor) {
        ...
        for (NSInteger i = 0; i<self.itemsArray.count; i++) {
            WZSegmentItem *item = self.itemsArray[i];
...
#pragma mark - WZSegmentItemDelegate
- (void)ItemStateChanged:(WZSegmentItem *)currentItem index:(NSInteger)index isSelected:(BOOL)isSelected
{
    if (self.itemsArray.count <=2) {
        return;
    }
