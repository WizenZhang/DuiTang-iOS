...
@interface WZSortDetailController ()<MJRefreshBaseViewDelegate,WZSortHeadDetailDelegate>
...
@implementation WZSortDetailController
...
- (void)setupHeadData
{
    NSString *path = [NSString stringWithFormat:CATEGORDOWNUPDETAIL_URL,_ID];
    // 1.创建请求管理对象
    ...
    [mgr GET:path parameters:nil
     success:^(AFHTTPRequestOperation *operation, id responseObject) {
         ...
         self.data = [[WZDetailHeadData alloc] initWithDictionary:data error:nil];
...
- (void)loadMoreData
{
    _page+=24;
        ...
        NSString *path = [NSString stringWithFormat:CATEDOWNDETAILDOWN_URL,_page,_ID];
        // 1.创建请求管理对象
        AFHTTPRequestOperationManager *mgr = [AFHTTPRequestOperationManager manager];
...
- (void)loadNewData
{
    NSString *path = [NSString stringWithFormat:CATEDOWNDETAILDOWN_URL,0,_ID];
    // 1.创建请求管理对象
    AFHTTPRequestOperationManager *mgr = [AFHTTPRequestOperationManager manager];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    //分类详情添加头部显示
    WZSortHeadDetail *headDeatail=[[WZSortHeadDetail alloc]init];
    
    //传递数据
    headDeatail.path=_path;
    headDeatail.data=_data;
    
    headDeatail.delegate=self;
...
{
    long countH=(self.data.sub_cates.count+2)/3;
    return 15+40*countH;
}
...
#pragma mark - WZSortHeadDetail Delegate
- (void)headViewClickButton:(WZSortHeadDetail *)headView
{
        WaterFLayout *layout = [[WaterFLayout alloc]init];
    
    WZCateDetailUpDetailController *uptDetail = [[WZCateDetailUpDetailController alloc] initWithCollectionViewLayout:layout];
        //传递ID数据模型
        uptDetail.ID=headView.theme_name;
        uptDetail.navigationItem.title=headView.name;
        [self.navigationController pushViewController:uptDetail animated:YES];
